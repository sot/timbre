...
if __name__ == "__main__":
    ...
    state_pairs = [({'duration1': t1, 'duration1_fraction': 1.0, 'sequence1': -1, 'obsid1': 99999, 'pitch': p1, 'roll': 0.0},
                   ...
                   for t1 in [120000, 150000, 180000]]
    date = '2020:001:00:00:00'
    t1 = DateTime().secs
    results = run_state_pairs(msid, model_specs[msid], init, limit, date, state_pairs)
    t2 = DateTime().secs
    print('took {} seconds, for {} state pairs'.format(t2 - t1, len(state_pairs)))
    pickle.dump(results, open('pftank2t_{}_{}.pkl'.format(datestamp, 5), 'wb'))
